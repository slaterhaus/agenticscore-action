...
function buildComment(result, specPath, failBelow) {
...
async function postPrComment(comment) {
    ...
    const { status } = await post(url, JSON.stringify({ body: comment }), {
...
async function run() {
    ...
    if (!fs.existsSync(absPath)) {
        error(`Spec file not found: ${absPath}`);
        process.exit(1);
    }
    ...
    const { status, body } = await post(`${apiUrl}/score`, specContent, {
    ...
    if (status === 422) {
        error(`Invalid OpenAPI spec: ${body}`);
        process.exit(1);
    }
    if (status !== 200) {
